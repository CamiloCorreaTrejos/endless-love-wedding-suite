    ...
    <div className="space-y-4 sm:space-y-5 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2">
          ...
          <button className="w-full sm:w-auto bg-white border border-stone-200 text-stone-500 px-4 py-2 rounded-xl text-[10px] font-bold flex items-center justify-center gap-2 hover:bg-stone-50 shadow-sm transition-all uppercase tracking-widest">
      ...
      <div className="flex flex-col lg:flex-row gap-3 items-center justify-between bg-white p-3 rounded-2xl border border-stone-100 shadow-sm">
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto">
          <label className="text-[9px] font-bold text-stone-400 uppercase tracking-widest ml-1 hidden sm:block">Categoría:</label>
          <select 
            value={filter} 
            onChange={(e) => setFilter(e.target.value)}
            className="flex-1 lg:flex-none px-3 py-2 bg-stone-50 border border-stone-100 rounded-xl text-[10px] font-bold text-stone-600 outline-none focus:border-[#C6A75E] transition-all min-w-[150px]"
          >
          ...
          <table className="w-full text-left min-w-[800px]">
            <thead className="bg-stone-50/50 border-b border-stone-100">
              <tr className="text-[9px] uppercase font-bold text-stone-400 tracking-widest">
                <th className="px-4 py-2.5 w-1/3">Invitados</th>
                <th className="px-4 py-2.5">Categoría</th>
                <th className="px-4 py-2.5">Asignación</th>
                <th className="px-4 py-2.5">Certeza</th>
                <th className="px-4 py-2.5">Estado</th>
                <th className="px-4 py-2.5 text-center">RSVP</th>
                ...
                return (
                  <tr key={inv.id} className="hover:bg-stone-50/40 transition-colors group">
                    <td className="px-4 py-3">
                      <p className="font-bold text-stone-800 text-xs md:text-sm serif leading-tight">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEditModal(inv)} className="p-1.5 text-stone-300 hover:text-[#C6A75E] hover:bg-white rounded-lg shadow-sm transition-all bg-stone-50 border border-stone-50">
                          <ICONS.Edit.type {...ICONS.Edit.props} size={14} />
                        </button>
                        <button onClick={() => onRemoveGuest(inv.id)} className="p-1.5 text-stone-200 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all">
...
const SummaryCard = ({ label, value, icon, color, badgeColor, alert = false }: any) => (
  <div className={`p-3 sm:p-4 rounded-2xl border border-stone-100 shadow-sm flex flex-col justify-between h-24 sm:h-28 transition-all hover:shadow-md ${color}`}>
    <div className="flex items-center justify-between">
    ...
    <div>
      ...
      <h4 className={`text-lg sm:text-xl font-bold ${badgeColor || 'text-stone-800'}`}>{value}</h4>
    </div>
  </div>
);
