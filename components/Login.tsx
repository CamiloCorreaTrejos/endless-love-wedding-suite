 ...
 // const FALLBACK_URL = "https://images.unsplash.com/photo-1511795409834-ef04bbd61622?q=80&w=2069&auto=format&fit=crop";
  ...
  return (
    <div className="h-screen w-full flex flex-col md:flex-row overflow-hidden bg-white select-none">
      ...
      <div className="w-full md:w-1/2 h-full flex items-center justify-center p-6 md:p-12 lg:p-16 overflow-y-auto md:overflow-hidden relative" style={{ backgroundColor: COLORS.primary }}>
        
        <div className="absolute inset-0 opacity-[0.02] pointer-events-none" 
             style={{ backgroundImage: `url("https://www.transparenttextures.com/patterns/cream-paper.png")` }} />

        <div className="w-full max-w-[440px] bg-white/90 backdrop-blur-md rounded-[3.5rem] p-10 lg:p-14 shadow-[0_40px_100px_-20px_rgba(15,26,46,0.12)] border border-white relative z-10 animate-in fade-in zoom-in-95 duration-1000">
          ...
          <div className="flex flex-col items-center gap-10 lg:gap-12">
            ...
            <div className="text-center space-y-6">
              ...
              <p className="text-[11px] italic text-stone-500 serif px-4 opacity-70 leading-relaxed">
                “Este espacio guarda cada detalle de nuestra historia.”
              </p>
              
              <div className="inline-flex items-center gap-3 px-4 py-2 bg-stone-50/50 border border-stone-100/80 rounded-full shadow-sm">
                <Calendar size={12} className="text-[#C6A75E]" />
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-[#0F1A2E] tabular-nums">{daysLeft}</span>
                  <span className="text-[8px] font-bold text-stone-400 uppercase tracking-widest">Días para el sí</span>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-10">
              <div className="space-y-6">
                <div className="space-y-2 group/field">
                  <label className="text-[8px] uppercase tracking-[0.2em] font-bold text-stone-400 ml-1">Acceso Privado</label>
                  <div className="relative">
                    <input 
                      ...
                      className="w-full pl-14 pr-6 py-5 bg-stone-50/50 border border-stone-100/50 rounded-2xl outline-none focus:border-[#C6A75E] focus:bg-white transition-all text-sm text-stone-800 font-medium shadow-inner"
                    />
                    <Mail size={16} className="absolute left-6 top-1/2 -translate-y-1/2 text-stone-300 group-focus-within/field:text-[#C6A75E] transition-colors" />
                  </div>
                </div>

                <div className="space-y-2 group/field">
                  <label className="text-[8px] uppercase tracking-[0.2em] font-bold text-stone-400 ml-1">Contraseña</label>
                  <div className="relative">
                    <input 
                      type="password" 
                      required
                      ...
                      className="w-full pl-14 pr-6 py-5 bg-stone-50/50 border border-stone-100/50 rounded-2xl outline-none focus:border-[#C6A75E] focus:bg-white transition-all text-sm text-stone-800 font-medium shadow-inner"
                    />
                    <Lock size={16} className="absolute left-6 top-1/2 -translate-y-1/2 text-stone-300 group-focus-within/field:text-[#C6A75E] transition-colors" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <button 
                  type="submit" 
                  disabled={isLoading}
                  className="w-full text-white font-bold py-6 rounded-[1.8rem] shadow-2xl shadow-[#0F1A2E]/20 transition-all active:scale-[0.98] flex items-center justify-center gap-3 disabled:bg-stone-300 relative group overflow-hidden"
                  style={{ backgroundColor: COLORS.accent }}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/10 to-white/0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                  {isLoading ? (
                    <span className="flex items-center gap-3 uppercase tracking-[0.25em] text-[10px]">
                      Verificando... <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    </span>
                  ) : (
                    <span className="flex items-center gap-3 uppercase tracking-[0.25em] text-[10px]">
                      Entrar a nuestra historia <Sparkles size={14} className="text-[#C6A75E]" />
                    </span>
                  )}
                </button>
